...
const SubjectForm = dynamic(() => import("./forms/SimpleSubjectForm"), {
  loading: () => <h1>Loading...</h1>,
});
  ...
  const Form = () => {
    ...
    useEffect(() => {
      if (state.success) {
        ...
        toast.error((state as any).message || `Failed to delete ${table}. Please try again.`);
        setIsDeleting(false);
      }
    }, [state, router]);
    ...
    return type === "delete" && id ? (
      <form onSubmit={handleSubmit} className="p-6 flex flex-col gap-6">
        ...
        <div className="text-center">
          <div className="w-12 h-12 bg-error-100 rounded-full flex items-center justify-center mx-auto mb-4">
            {isDeleting ? (
              <div className="w-6 h-6 border-2 border-error-600 border-t-transparent rounded-full animate-spin"></div>
            ) : (
          ...
          <h3 className="text-lg font-semibold text-neutral-900 mb-2">
            {isDeleting ? `Deleting ${table}...` : `Delete ${table}`}
          </h3>
          <p className="text-neutral-600">
            {isDeleting
              ? `Please wait while we delete this ${table}.`
        ...
        <div className="flex gap-3 justify-end">
          ...
          <button
            type="submit"
            disabled={isDeleting}
            className="px-4 py-2 text-sm font-medium text-white bg-error-600 rounded-lg hover:bg-error-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
          >
            {isDeleting && (
      ...
      forms[table] ? forms[table](setOpen, type, data, relatedData) : (
        <div className="p-6 text-center">
          ...
          <h3 className="text-lg font-semibold text-neutral-900 mb-2">Form Not Available</h3>
          <p className="text-neutral-600">Form for {table} is not yet implemented</p>
        </div>
      )
    ) : (
      <div className="p-6 text-center">
